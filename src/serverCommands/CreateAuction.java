...
public class CreateAuction extends ServerCommand {
	...
	private boolean checkValues(){
		if(duration < 0)
			return false;
		try {
		  con.jAuctionServer.resourceStore.getResource(resource_id);
	...
	public void run(){
	  if(authenticate(auth_key)){
			...
			int auction_id;
	  		HashMap data = new HashMap();
			if (checkValues()){
				Resource resource = con.jAuctionServer.resourceStore.getResource(resource_id);
				User user = con.user;
				Auction auc = con.jAuctionServer.auctionStore.addAuction(amount,resource, duration, user, price);
