...
public class GetResource extends ServerCommand {
	...
	public void run(){
	  		HashMap data = new HashMap();
	  		Resource resource;
	  		if(con.jAuctionServer.resourceStore.getResources().containsKey(resource_id)){
	  			data.put("resource_id", resource_id);
	  			data.put("name", con.jAuctionServer.resourceStore.getResource(resource_id).getName());
	  		}
